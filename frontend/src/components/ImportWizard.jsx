...
export default function ImportWizard({ backendUrl, groupId, onImportComplete, currentUser }) {
  ...
  const getSeverityStyle = (sev) => {
    if (sev === "error" || sev === "High") return { bg: "bg-red-500/10", color: "text-red-400", border: "border-red-500/20" };
    if (sev === "warning" || sev === "Medium") return { bg: "bg-amber-500/10", color: "text-amber-400", border: "border-amber-500/20" };
    return { bg: "bg-blue-500/10", color: "text-blue-400", border: "border-blue-500/20" };
  };
  ...
  return (
    <div className="max-w-[1200px] mx-auto py-4 animate-fadeIn relative z-10">
      ...
      {/* Title */}
      <div className="mb-6">
        <h2 className="text-xl font-bold text-white tracking-tight">Import & Anomaly Detection</h2>
        <p className="text-sm text-zinc-400">Meera's View</p>
      </div>
              ...
              <React.Fragment key={i}>
                <div className="flex flex-col items-center gap-2">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold transition-all ${
                    isCompleted ? "bg-[#00d8a5] text-[#09090b]" : isCurrent ? "bg-[#00d8a5] text-[#09090b] ring-4 ring-[#00d8a5]/20" : "bg-[#27272a] text-zinc-500"
                  }`}>
                    {isCompleted ? <Check className="w-3 h-3" /> : num}
                  </div>
                  <span className={`text-[10px] font-medium ${isCurrent ? "text-[#00d8a5]" : "text-zinc-500"}`}>{label}</span>
                </div>
                {i < stepLabels.length - 1 && (
                  <div className={`flex-1 h-px mx-4 ${isCompleted ? "bg-[#00d8a5]" : "bg-[#27272a]"}`} />
                )}
              </React.Fragment>
            );
          })}
      ...
      {error && (
        <div className="p-4 rounded border border-red-500/20 bg-red-500/10 flex gap-3 mb-6 items-center text-sm text-red-400">
      ...
      {/* STEP 1: UPLOAD */}
      {step === 1 && (
        <div className="card p-12 text-center max-w-xl mx-auto flex flex-col items-center justify-center border-dashed border-2 border-[#27272a] bg-[#18181b] hover:border-[#00d8a5] transition-colors">
          <div className="w-16 h-16 rounded bg-[#00d8a5]/10 border border-[#00d8a5]/20 flex items-center justify-center mb-6">
            {loading ? <Loader2 className="w-8 h-8 animate-spin text-[#00d8a5]" /> : <Upload className="w-8 h-8 text-[#00d8a5]" />}
          </div>
          <h2 className="text-xl font-bold mb-2 text-white">Upload Expenses CSV</h2>
      ...
      {step === 2 && (
        <div className="card overflow-hidden">
          <div className="p-5 border-b border-[#27272a] bg-[#18181b] flex justify-between items-center">
            <div>
              <div className="flex items-center gap-2">
                <CheckCircle className="w-4 h-4 text-[#00d8a5]" />
                <h3 className="font-bold text-white">CSV parsed successfully</h3>
              </div>
              <p className="text-xs text-zinc-400 mt-1">
                {draftExpenses.length} rows processed • <span className="font-semibold text-amber-400">{anomalies.length} anomalies detected</span>
              </p>
            </div>
            ...
            <table className="w-full text-left text-sm border-collapse">
              <thead className="bg-[#09090b] border-b border-[#27272a]">
                <tr>
                  ...
                  <th className="py-3 px-5 font-semibold text-xs text-zinc-500 text-center w-32">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#27272a] bg-[#18181b]">
                {draftExpenses.map((exp) => {
                  ...
                  if (rowAnoms.length === 0 && res.action === "APPROVE") return null;

                  return rowAnoms.map((anom, idx) => {
                    const style = getSeverityStyle(anom.severity);
                    return (
                      <tr key={`${exp.rowNumber}-${idx}`} className="hover:bg-[#27272a]/50 transition-colors">
                        <td className="py-4 px-5 font-medium text-zinc-300">{exp.rowNumber}</td>
                        ...
                        <td className="py-4 px-5">
                          <p className={`text-white ${res.action === "REJECT" ? "line-through opacity-50" : ""}`}>{anom.description}</p>
                          {res.action === "EDIT" && <p className="text-[10px] text-amber-400 mt-1 font-medium">Pending edits will be applied.</p>}
                        </td>
                        <td className="py-4 px-5">
                          <div className="flex items-center justify-center gap-1 border border-[#27272a] rounded p-1 bg-[#09090b] shadow-sm">
                            <button onClick={() => toggleRowAction(exp.rowNumber, "APPROVE")} className={`p-1.5 rounded ${res.action === "APPROVE" ? "bg-emerald-500/20 text-[#00d8a5]" : "text-zinc-500 hover:bg-[#18181b]"}`} title="Approve">
                              <Check className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => toggleRowAction(exp.rowNumber, "REJECT")} className={`p-1.5 rounded ${res.action === "REJECT" ? "bg-red-500/20 text-red-400" : "text-zinc-500 hover:bg-[#18181b]"}`} title="Reject">
                              <X className="w-3.5 h-3.5" />
                            </button>
                            <button onClick={() => startEditing(exp.rowNumber)} className={`p-1.5 rounded ${res.action === "EDIT" ? "bg-amber-500/20 text-amber-400" : "text-zinc-500 hover:bg-[#18181b]"}`} title="Edit">
          ...
          <div className="p-4 bg-[#09090b] border-t border-[#27272a] text-xs text-zinc-600 text-center">
        ...
        <div className="modal-overlay">
          <div className="card max-w-lg w-full p-6 overflow-y-auto max-h-[90vh]">
            <div className="flex justify-between items-center mb-4 pb-3 border-b border-[#27272a]">
              <h3 className="text-lg font-bold text-white tracking-tight">Edit Row {editingRow}</h3>
              <button onClick={() => { setEditingRow(null); setEditFormData(null); }} className="text-zinc-500 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-xs block mb-1 font-semibold text-zinc-400">Description</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs block mb-1 font-semibold text-zinc-400">Amount</label>
                ...
                <div>
                  <label className="text-xs block mb-1 font-semibold text-zinc-400">Currency</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs block mb-1 font-semibold text-zinc-400">Payer</label>
                ...
                <div>
                  <label className="text-xs block mb-1 font-semibold text-zinc-400">Date</label>
      ...
      {/* STEP 3: REPORT */}
      {step === 3 && (
        <div className="card p-8 text-center max-w-2xl mx-auto border-t-2 border-t-[#00d8a5]">
          <div className="w-16 h-16 rounded bg-[#00d8a5]/10 border border-[#00d8a5]/20 flex items-center justify-center mx-auto mb-4">
            <Check className="w-8 h-8 text-[#00d8a5]" />
          </div>
          ...
          <div className="flex justify-center gap-4">
            ...
            <button onClick={() => {
              ...
              doc.autoTable({ startY: 36, head: [["Row", "Description", "Action Taken"]], body: tableData, theme: 'grid', styles: { fontSize: 9 }, headStyles: { fillColor: [0, 216, 165] } });
              doc.save("report.pdf");
