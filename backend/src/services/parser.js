        ...
        try {
          ...
          results.forEach((row, index) => {
            ...
            if (splitType === "percentage") {
              ...
              // Anomaly: Percentage doesn't sum to 100%
              ...
              amountMatches.forEach((match) => {
                splits.push({
                  user: match.user,
                  share: match.value, // value represents the unequal amount logged
                  amount: match.value,
                });
              });
            } else {
              // Equal split
              // Anomaly: Equal split has details anyway (Row 42)
              if (splitDetailsText) {
                rowAnomalies.push({
              ...
              // Split equally among the active split members
              const activeMembers = splitMembers.length > 0 ? splitMembers : KNOWN_USERS.filter((u) => u !== "Dev"); // default fallback if empty
            ...
            // 13. Anomaly: Duplicate entries (exact or fuzzy duplicate)
            ...
            // 14. Package everything as a parsed item
            const parsedExpense = {
              rowNumber: rowNum,
