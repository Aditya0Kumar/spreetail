...
async function computeGroupBalances(groupId) {
  ...
  expenses.forEach(exp => {
    ...
    // Amount is in base currency (INR)
    const amountInINR = exp.amount * exp.exchangeRate;
    ...
    exp.splits.forEach(split => {
      const splitUser = split.user.username;
      const owedInINR = split.amount * exp.exchangeRate;
  ...
  // Fetch all payments (settlements)
  ...
  payments.forEach(pay => {
    const sender = pay.payer.username;
    const recipient = pay.payee.username;
    const amountInINR = pay.amount * pay.exchangeRate;
  ...
  // Net = (Paid + Sent) - (Owed + Received)
  // i.e., Paid represents money you spent for others. Sent is payments you made to settle.
  // Owed is your share of bills. Received is money others paid you back.
  // Positive Net means you are owed money overall. Negative Net means you owe money overall.
  Object.keys(balances).forEach(username => {
    const userBal = balances[username];
...
async function getPairwiseLedger(groupId, userA_Name, userB_Name) {
  const userA = await prisma.user.findUnique({ where: { username: userA_Name } });
  const userB = await prisma.user.findUnique({ where: { username: userB_Name } });
  ...
  // 3. Fetch payments from B to A
  const paymentsFromBToA = await prisma.payment.findMany({
    where: {
      groupId,
      payerId: userB.id,
      payeeId: userA.id
  ...
  // 4. Fetch payments from A to B
  const paymentsFromAToB = await prisma.payment.findMany({
    where: {
      groupId,
      payerId: userA.id,
      payeeId: userB.id
  ...
  expPaidByA.forEach(exp => {
    ...
    const amountInINR = split.amount * exp.exchangeRate;
    ledgerItems.push({
      id: `exp-split-a-${exp.id}`,
      type: "expense_split",
      date: exp.date,
      description: exp.description,
      payer: userA.username,
      debtor: userB.username,
  ...
  expPaidByB.forEach(exp => {
    ...
    ledgerItems.push({
      ...
      date: exp.date,
      description: exp.description,
      payer: userB.username,
      debtor: userA.username,
      originalAmount: split.amount,
      currency: exp.currency,
      exchangeRate: exp.exchangeRate,
      amountInINR: Math.round(amountInINR * 100) / 100,
  ...
  // Payments from B to A reduces B's debt (or acts as A receiving money)
  paymentsFromBToA.forEach(pay => {
    ...
    ledgerItems.push({
      ...
      type: "payment",
      date: pay.date,
      description: "Direct payment: " + userB.username + " paid " + userA.username,
      payer: userB.username,
      debtor: userA.username,
  ...
  paymentsFromAToB.forEach(pay => {
    ...
    ledgerItems.push({
      ...
      type: "payment",
      date: pay.date,
      description: "Direct payment: " + userA.username + " paid " + userB.username,
      payer: userA.username,
      debtor: userB.username,
      originalAmount: pay.amount,
      currency: pay.currency,
  ...
  // Running balance from User B's perspective (positive = B owes A, negative = A owes B)
