...
This document lists the final database schema layout and details all **19 deliberate data anomalies** discovered in the CSV sheet, along with our programmatic resolution policies.

---

## 1. Relational Database Schema

We use **Prisma ORM** connecting to a **Supabase PostgreSQL** relational database. The schema structure is as follows:
...
1. **User Table:**
   ...
   - `email` (String, Unique)
   - *Relations:* memberships, expensesPaid, splits, paymentsSent, paymentsReceived.

2. **Group Table:**
   - `id` (Int, PK, Autoincrement)
   - `name` (String)
   - `createdAt` (DateTime)
   - *Relations:* memberships, expenses, payments.

3. **GroupMembership Table (Sam's View / History Tracker):**
   - `id` (Int, PK, Autoincrement)
   - `groupId` (Int, FK to Group)
   - `userId` (Int, FK to User)
   - `joinedAt` (DateTime)
   - `leftAt` (DateTime, Nullable)
   - *Purpose:* Tracks active membership spans to exclude flatmates from splits if they were not living in the house when the expense occurred.
...
5. **ExpenseSplit Table:**
   ...
   - `userId` (Int, FK to User)
   - `amount` (Double)
   - `share` (Double) - raw share weights, percentages, or unequal amounts.

6. **Payment Table (Settle up logs):**
   - `id` (Int, PK, Autoincrement)
   - `groupId` (Int, FK to Group)
   - `payerId` (Int, FK to User)
   - `payeeId` (Int, FK to User)
   - `amount` (Double)
   - `currency` (String)
   - `exchangeRate` (Double)
   - `date` (DateTime)

7. **ImportJob & ImportAnomaly Tables:**
   - Tracks uploaded files and logs the final status and descriptions of resolved anomalies.

---

## 2. Anomaly Resolution Log

Our importer scanned `expenses_export.csv` and logged **19 separate anomalies**:
...
| **9** | `PAYER_NAME_CASING` | `priya` in lowercase instead of `Priya`. | Lowercase name. | **NORMALIZED** to `Priya`. |
...
| **14** | `SETTLEMENT_AS_EXPENSE` | Rohan, "Rohan paid Aisha back", 5000 INR. | split_type empty, description indicates a payback. | **CONVERTED** to `Payment` record instead of split Expense. |
| **15** | `INVALID_PERCENTAGE_SUM` | Aisha, "Pizza Friday", percentages sum to 110%. | Split type is percentage, but sum is off. | **RE-SCALED** percentages to sum to 100% (divided each by 1.1). |
...
| **25** | `DUPLICATE_EXPENSE` | Rohan, "Thalassa dinner", 2450 INR. | Conflict duplicate of Row 24 ("Dinner at Thalassa", Aisha, 2400). | **REJECTED/DELETED** (Aisha's row preserved, or resolved manually). |
...
| **38** | `SETTLEMENT_AS_EXPENSE` | Sam, "Sam deposit share", 15000 INR. | Sam pays deposit to Aisha. | **CONVERTED** to `Payment` record. |
